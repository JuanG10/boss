...
func calc_cohesion(flock_mates):
	...
	if flock_mates.size() > 1:
		...
		var _u    = acos(acc.dot(enemy.velocity.normalized())) / PI
		steer   += acc
		...
		for f in flock_mates:
			if f != enemy:
				if (f.global_position - enemy.global_position).length() < 20:
					steer += (enemy.global_position - f.global_position).normalized()
...
func calc_alignment(flock_mates):
	...
	if flock_mates.size() > 1:
		...
		var _v    = acos(acc.dot(enemy.velocity.normalized())) / PI
		steer   += acc
